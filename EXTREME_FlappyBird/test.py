...
class Player:
    ...
    def calculateTrajectory(self, iterations: int):
        trajectory = []
        currentY = self.y
        currentX = self.x
        ...
        for i in range(iterations):
            currentVel += playerGravity
            currentY += currentVel
            currentX += pipeSpeed
            trajectory.append((currentX, currentY))

        return trajectory

    def drawLines(self, trajectory: list, color, width: int):
        pygame.draw.lines(screen, color, False, trajectory, width)
